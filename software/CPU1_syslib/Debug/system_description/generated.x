 ...
 *
 * Generated: 2015-11-16 16:38:04.429
 *
 */
...
MEMORY
{
    reset : ORIGIN = 0x00200000, LENGTH = 32
    sdram_UNUSED : ORIGIN = 0x04000000, LENGTH = 16777248
    sdram : ORIGIN = 0x05000020, LENGTH = 16777184
    cfi_UNUSED : ORIGIN = 0x00000000, LENGTH = 2097152
    cfi : ORIGIN = 0x00200020, LENGTH = 2097120
}

    /* Define symbols for each memory base-address */
 __alt_mem_sdram = 0x04000000 ;
 __alt_mem_cfi = 0x00000000 ;
...
SECTIONS
{
   ...
   PROVIDE (__flash_exceptions_start = LOADADDR(.exceptions));
    ...
    .cfi LOADADDR (.sdram) + SIZEOF (.sdram): AT (LOADADDR (.sdram) + SIZEOF (.sdram))
    {
        ...
        *(.cfi .cfi.*)
        . = ALIGN(32 / 8);
        PROVIDE (_alt_partition_cfi_end = ABSOLUTE(.));
    } > cfi

    PROVIDE (_alt_partition_cfi_load_addr = LOADADDR(.cfi));
